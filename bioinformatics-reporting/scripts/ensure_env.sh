#!/usr/bin/env bash
# Persistent reusable venv for the bioinformatics-reporting skill.
#
# Cache location:
#   ~/.cache/cursor-skills/bioinformatics-reporting/
#   ├── README.txt
#   ├── requirements.txt.sha256
#   └── venv/
#
# Backend: python -m venv (pure-Python deps; Quarto CLI is a host binary when available).
#
# Force rebuild:
#   bash scripts/ensure_env.sh --force-rebuild
#   rm -rf ~/.cache/cursor-skills/bioinformatics-reporting
...
find_host_python() {
  ...
  die "Need Python 3.10+ on PATH to create the skill venv."
}
...
bioinformatics-reporting persistent environment
===============================================
...
Optional host tools (not installed by this helper):
  - quarto for HTML/PDF rendering
  - pdflatex (or xelatex) for PDF output via Quarto
  - SVG support in LaTeX (e.g. inkscape) when using CAB_logo.svg on the title page
...
env_ready() {
  ...
  py="$(venv_python)"
  [[ -n "${py}" && -x "${py}" ]] || return 1
  "${py}" -c "import pandas, yaml, PIL, jinja2" >/dev/null 2>&1 || return 1
...
create_env() {
  ...
  write_cache_readme
  "${py}" -c "import pandas, yaml, PIL, jinja2" || die "Key imports failed after install"
