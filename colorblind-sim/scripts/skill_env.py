...
The reusable environment lives outside the repo at:
    ~/.cache/cursor-skills/colorblind-sim/venv/

Set COLORBLIND_SIM_SKIP_ENV_BOOTSTRAP=1 to bypass (e.g. pytest).
...
ENV_ACTIVE_VAR = "COLORBLIND_SIM_ENV_ACTIVE"
SKIP_VAR = "COLORBLIND_SIM_SKIP_ENV_BOOTSTRAP"
...
def bootstrap() -> None:
    ...
    updated so ``cbviz`` / ``cbviz-fast`` resolve. Otherwise this process is
