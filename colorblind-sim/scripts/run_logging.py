...
"""Shared reproducibility logging helpers for colorblind-sim scripts."""
...
def configureLogging(logFile: Optional[Path] = None) -> None:
    ...
    Args:
        logFile (Optional[Path]): File path for a persistent log under ``logs/``.
            When ``None`` only a stream handler is attached.
...
def writeAgentArtifacts(
    ...
    *,
    requestEnvVar: str = "COLORBLIND_SIM_AGENT_REQUEST",
    workflowEnvVar: str = "COLORBLIND_SIM_AGENT_WORKFLOW",
...
def writeRunMetadata(
    ...
    *,
    ...
    metadata = {
        ...
        "citation_keys": ["cbviz", "colorspacious"],
    }
